...
export function TradeListComponent() {
    ...
    const getTradeList = async () => {
        try {
            const resp = await axios.get(
                "http://localhost:8080/show/tradeList"
            );
    ...
    const getUserData = async (user_id) => {
        try {
            const userResp = await axios.get(
                `http://localhost:8080/show/${user_id}`
            );
    ...
    }, [currentPage, pageSize, tradeList.length < 1, pagedData.length < 1]);
    ...
    return (
        <div className="app">
            ...
            <button className="add-button" onClick={addTradeButton}>
                글 작성하기
            </button>
                        ...
                        {pagedData.map((trade, index) => (
                            <tr key={index}>
                                <th scope="row" className="category-box">
                                    {getCategoryText(trade.category)}
                                </th>
                                <td
                                    onClick={(event) => {
                                        if (event.target.tagName === "TD") {
                                            navigate(
                                                `/trade/detail/${trade.comm_id}`
                                            );
                                        }
                                    }}
